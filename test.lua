	...
	local CONSTANTS = {
		...
		RETRY_DELAY = 3,
		MAX_RETRIES = 3,
		SAFE_TELEPORT_OFFSET = Vector3.new(0, 5, 0),
		PLAYER_TELEPORT_OFFSET = Vector3.new(3, 2, 3),
		BOAT_SPAWN_DELAY = 0.5,
		ROD_EQUIP_DELAY = 0.5
	}
	...
	for _, island in ipairs(tpFolder:GetChildren()) do
		if island:IsA("BasePart") then
			...
			-- Improved teleport with safety checks
			btn.MouseButton1Click:Connect(function()
				local success = pcall(function()
					local localPlayer = game.Players.LocalPlayer
					if not localPlayer.Character or not localPlayer.Character:FindFirstChild("HumanoidRootPart") then
						warn("❌ Character not ready for teleport")
						return
					end
					
					-- Add safe offset above the island
					local safePosition = island.CFrame.Position + CONSTANTS.SAFE_TELEPORT_OFFSET
					local safeCFrame = CFrame.new(safePosition, island.CFrame.LookVector)
				...
				if not success then
					warn("🔴 Failed to teleport to: " .. island.Name)
				end
			end)
			index += 1
		end
	...
	for _, targetPlayer in ipairs(charFolder:GetChildren()) do
		if targetPlayer:IsA("Model") and targetPlayer.Name ~= game.Players.LocalPlayer.Name then
			local btn = Instance.new("TextButton")
			...
			-- Improved player teleport with validation
			btn.MouseButton1Click:Connect(function()
				local success = pcall(function()
					local localPlayer = game.Players.LocalPlayer
					if not localPlayer.Character or not localPlayer.Character:FindFirstChild("HumanoidRootPart") then
						warn("❌ Your character not ready for teleport")
						return
					end
					
					-- Check if target player still exists and has valid character
					local currentTarget = charFolder:FindFirstChild(targetPlayer.Name)
					if not currentTarget or not currentTarget:FindFirstChild("HumanoidRootPart") then
						warn("❌ Target player not found or invalid: " .. targetPlayer.Name)
						return
					end
					
					-- Teleport with safe offset
					local targetPos = currentTarget.HumanoidRootPart.CFrame.Position
					local safeCFrame = CFrame.new(targetPos + CONSTANTS.PLAYER_TELEPORT_OFFSET)
				...
				if not success then
					warn("🔴 Failed to teleport to player: " .. targetPlayer.Name)
				end
			end)
			index += 1
		end
	...
	local fishingRetryCount = 0
	
	-- Helper function untuk safe remote calls
	function safeRemoteCall(remote, method, ...)
	...
	-- Helper function untuk cek rod equipped
	function hasValidRod()
	...
	function toggleFishing(state)
		if state == true then
			...
			-- Spawn thread AutoFishing dengan improved error handling
			autoFishThread = task.spawn(function()
				while _G.AutoFishing do
					local success = pcall(function()
						-- Pastikan character dan connection tersedia
						local char = player.Character
						...
						-- Auto-equip rod jika diperlukan
						if not hasValidRod() then
							print("🔧 Equipping fishing rod...")
							safeRemoteCall(CancelFishing, "InvokeServer")
							task.wait(0.1)
							safeRemoteCall(EquipRod, "FireServer", 1)
							task.wait(CONSTANTS.ROD_EQUIP_DELAY) -- Wait for equip
						end

						-- Proses memancing jika rod sudah siap
						if hasValidRod() then
							safeRemoteCall(ChargeRod, "InvokeServer", workspace:GetServerTimeNow())
							task.wait(CONSTANTS.ROD_EQUIP_DELAY) -- Slightly longer wait
							safeRemoteCall(RequestFishing, "InvokeServer", -1.2379989624023438, 0.9800224985802423)
							task.wait(0.4)
							safeRemoteCall(FishingComplete, "FireServer")
							
							fishingRetryCount = 0 -- Reset counter on success
							print("✅ Fishing cycle completed")
						else
							warn("❌ Rod not equipped, retrying...")
							task.wait(1)
						end
					end)
					
					-- Handle errors dengan retry mechanism
					if not success then
						fishingRetryCount = fishingRetryCount + 1
						warn("🔴 AutoFishing error, attempt " .. fishingRetryCount .. "/" .. CONSTANTS.MAX_RETRIES)
						
						if fishingRetryCount >= CONSTANTS.MAX_RETRIES then
							warn("💀 Max retry attempts reached, stopping AutoFishing")
							_G.AutoFishing = false
							AutoFishButton.Text = "OFF"
							break
						end
						
						task.wait(CONSTANTS.RETRY_DELAY) -- Wait longer on error
					else
						task.wait(CONSTANTS.FISHING_DELAY) -- Normal delay between cycles
					end
				end
			...
			pcall(function()
				safeRemoteCall(CancelFishing, "InvokeServer")
				safeRemoteCall(UnEquipRod, "FireServer")
			end)
			
			-- Cancel thread jika masih berjalan
	...
	function showPanel(pageName)
		...
		if selectedPanel then
			...
			-- Ganti judulnya
			if Tittle then
				Tittle.Text = string.upper(pageName)
			end
			print("📋 Switched to panel: " .. pageName)
		else
			print("❌ Panel not found: " .. tostring(pageName))
		end
	end

	-- Improved Boat Management
	local currentBoat = nil
	
	-- Helper function untuk spawn boat dengan error handling
	function safeSpawnBoat(boatType)
		local success = pcall(function()
		-- Despawn current boat first
		if currentBoat then
			print("🚢 Despawning current boat: " .. currentBoat)
			safeRemoteCall(despawnBoat, "InvokeServer")
			task.wait(CONSTANTS.BOAT_SPAWN_DELAY) -- Wait for despawn
		end			print("🚢 Spawning boat: " .. boatType)
			safeRemoteCall(spawnBoat, "InvokeServer", boatType)
			currentBoat = boatType
			print("✅ Successfully spawned: " .. boatType)
		end)
		
		if not success then
			warn("🔴 Failed to spawn boat: " .. boatType)
			return false
		end
		return true
	end
	
	-- Helper function untuk despawn boat
	function safeDespawnBoat()
		local success = pcall(function()
			if currentBoat then
				print("🚢 Despawning boat: " .. currentBoat)
				safeRemoteCall(despawnBoat, "InvokeServer")
				currentBoat = nil
				print("✅ Boat despawned successfully")
			else
				print("ℹ️ No boat to despawn")
			end
		end)
	...
	SellAllButton.MouseButton1Click:Connect(function()
		local success = pcall(function()
			safeRemoteCall(sellAll, "InvokeServer")
			print("💰 Sell All executed")
		end)
		
		if not success then
			warn("🔴 Failed to sell items")
		end
	end)

	-- Boat Management Button Events
	DespawnBoatButton.MouseButton1Click:Connect(function()
		safeDespawnBoat()
	end)

	SmallBoatButton.MouseButton1Click:Connect(function()
		safeSpawnBoat("SmallBoat")
	end)

	KayakBoatButton.MouseButton1Click:Connect(function()
		safeSpawnBoat("Kayak")
	end)

	JetskiBoatButton.MouseButton1Click:Connect(function()
		safeSpawnBoat("Jetski")
	end)

	HighfieldBoatButton.MouseButton1Click:Connect(function()
		safeSpawnBoat("HighfieldBoat")
	end)

	SpeedBoatButton.MouseButton1Click:Connect(function()
		safeSpawnBoat("SpeedBoat")
	end)

	FishingBoatButton.MouseButton1Click:Connect(function()
		safeSpawnBoat("FishingBoat")
	end)
