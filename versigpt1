...
function ZayrosFishingGUI:createSidebar()
    ...
    local buttons = {"Main", "Player", "Teleport", "Boats", "Rod Status", "Settings"}
...
function ZayrosFishingGUI:createRodStatusPage()
    ...
    rodPage.BackgroundTransparency = 1
    rodPage.Visible = false

    local scrollFrame = Instance.new("ScrollingFrame")
    scrollFrame.Parent = rodPage
    ...
    -- Rod Info
    local rodInfoFrame = createFrame(scrollFrame, "RodInfoFrame", 
        UDim2.new(1, -20, 0, 120), 
        UDim2.new(0, 10, 0, 10)
    )

    local rodNameLabel = Instance.new("TextLabel")
    rodNameLabel.Parent = rodInfoFrame
    rodNameLabel.Size = UDim2.new(1, 0, 0, 30)
    rodNameLabel.Position = UDim2.new(0, 0, 0, 0)
    rodNameLabel.BackgroundTransparency = 1
    rodNameLabel.Text = "Rod: -"
    rodNameLabel.TextColor3 = COLORS.Text
    rodNameLabel.TextScaled = true
    rodNameLabel.Font = Enum.Font.SourceSansBold

    -- Luck
    local luckLabel = Instance.new("TextLabel")
    luckLabel.Parent = rodInfoFrame
    luckLabel.Size = UDim2.new(0.4, 0, 0, 30)
    luckLabel.Position = UDim2.new(0, 0, 0, 40)
    ...
    luckBox.Parent = rodInfoFrame
    luckBox.Size = UDim2.new(0.2, 0, 0, 30)
    luckBox.Position = UDim2.new(0.45, 0, 0, 40)
    luckBox.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
    luckBox.BorderSizePixel = 0
    luckBox.Text = "0"
    luckBox.TextColor3 = COLORS.Text
    luckBox.TextScaled = true
    luckBox.Font = Enum.Font.SourceSans

    local luckSetBtn = createButton(rodInfoFrame, "SetLuckBtn", "Set", 
        UDim2.new(0.15, 0, 0, 30),
        UDim2.new(0.7, 0, 0, 40)
    )
    self:addButtonHoverEffect(luckSetBtn)

    -- Speed
    local speedLabel = Instance.new("TextLabel")
    speedLabel.Parent = rodInfoFrame
    ...
    speedBox.Parent = rodInfoFrame
    speedBox.Size = UDim2.new(0.2, 0, 0, 30)
    speedBox.Position = UDim2.new(0.45, 0, 0, 80)
    ...
    local speedSetBtn = createButton(rodInfoFrame, "SetSpeedBtn", "Set", 
        UDim2.new(0.15, 0, 0, 30),
        UDim2.new(0.7, 0, 0, 80)
    )
    self:addButtonHoverEffect(speedSetBtn)

    -- Weight
    local weightLabel = Instance.new("TextLabel")
    weightLabel.Parent = rodInfoFrame
    weightLabel.Size = UDim2.new(0.4, 0, 0, 30)
    weightLabel.Position = UDim2.new(0, 0, 0, 120)
    ...
    weightBox.Parent = rodInfoFrame
    weightBox.Size = UDim2.new(0.2, 0, 0, 30)
    weightBox.Position = UDim2.new(0.45, 0, 0, 120)
    weightBox.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
    weightBox.BorderSizePixel = 0
    weightBox.Text = "0"
    weightBox.TextColor3 = COLORS.Text
    weightBox.TextScaled = true
    weightBox.Font = Enum.Font.SourceSans

    local weightSetBtn = createButton(rodInfoFrame, "SetWeightBtn", "Set", 
        UDim2.new(0.15, 0, 0, 30),
        UDim2.new(0.7, 0, 0, 120)
    )
    self:addButtonHoverEffect(weightSetBtn)

    -- Fungsi update info rod
    local function updateRodInfo()
        local rod = self.player.Character and self.player.Character:FindFirstChild("!!!EQUIPPED_TOOL!!!")
        if rod then
            rodNameLabel.Text = "Rod: " .. rod.Name
            luckLabel.Text = "Luck: " .. tostring(rod:FindFirstChild("Luck") and rod.Luck.Value or "-")
            speedLabel.Text = "Speed: " .. tostring(rod:FindFirstChild("Speed") and rod.Speed.Value or "-")
            weightLabel.Text = "Weight: " .. tostring(rod:FindFirstChild("Weight") and rod.Weight.Value or "-")
            luckBox.Text = tostring(rod:FindFirstChild("Luck") and rod.Luck.Value or "0")
            speedBox.Text = tostring(rod:FindFirstChild("Speed") and rod.Speed.Value or "0")
            weightBox.Text = tostring(rod:FindFirstChild("Weight") and rod.Weight.Value or "0")
        else
            rodNameLabel.Text = "Rod: -"
            luckLabel.Text = "Luck: -"
            speedLabel.Text = "Speed: -"
    ...
    -- Update info saat halaman dibuka
    rodPage:GetPropertyChangedSignal("Visible"):Connect(function()
        if rodPage.Visible then
            updateRodInfo()
        end
    end)

    -- Tombol set Luck
    luckSetBtn.MouseButton1Click:Connect(function()
        local rod = self.player.Character and self.player.Character:FindFirstChild("!!!EQUIPPED_TOOL!!!")
        if rod and rod:FindFirstChild("Luck") then
            rod.Luck.Value = tonumber(luckBox.Text) or 0
            self:createNotification("Luck rod diubah!", "success", 1)
            updateRodInfo()
        else
            self:createNotification("Rod/Luck tidak ditemukan!", "error", 2)
        end
    end)
    ...
    speedSetBtn.MouseButton1Click:Connect(function()
        local rod = self.player.Character and self.player.Character:FindFirstChild("!!!EQUIPPED_TOOL!!!")
        if rod and rod:FindFirstChild("Speed") then
            rod.Speed.Value = tonumber(speedBox.Text) or 0
            self:createNotification("Speed rod diubah!", "success", 1)
            updateRodInfo()
        else
            self:createNotification("Rod/Speed tidak ditemukan!", "error", 2)
        end
    end)

    -- Tombol set Weight
    weightSetBtn.MouseButton1Click:Connect(function()
        local rod = self.player.Character and self.player.Character:FindFirstChild("!!!EQUIPPED_TOOL!!!")
        if rod and rod:FindFirstChild("Weight") then
            rod.Weight.Value = tonumber(weightBox.Text) or 0
            self:createNotification("Weight rod diubah!", "success", 1)
            updateRodInfo()
        else
            self:createNotification("Rod/Weight tidak ditemukan!", "error", 2)
        end
    end)
