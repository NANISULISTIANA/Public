...
local Settings = {
    ...
    AutoBaitUse = false,
    SmartFishing = false,
    -- New Fishing Stats Modification
    ModifyFishingStats = false,
    CustomFishingLuck = 175,
    CustomShinyChance = 4,
    CustomMutationChance = 0
}
...
-- ===================================================================
--                    FISHING STATS MODIFICATION
-- ===================================================================
local function modifyFishingStats()
    safeCall(function()
        if not Settings.ModifyFishingStats then return end
        
        -- Try to find and modify fishing stats remotes
        local statsRemotes = {
            Rs.Packages._Index["sleitnick_net@0.2.0"].net:FindFirstChild("RF/SetFishingLuck"),
            Rs.Packages._Index["sleitnick_net@0.2.0"].net:FindFirstChild("RF/SetShinyChance"),
            Rs.Packages._Index["sleitnick_net@0.2.0"].net:FindFirstChild("RF/SetMutationChance"),
            Rs.Packages._Index["sleitnick_net@0.2.0"].net:FindFirstChild("RF/ModifyPlayerStats")
        }
        
        -- Try invoking stat modification remotes
        for _, remote in pairs(statsRemotes) do
            if remote and remote:IsA("RemoteFunction") then
                pcall(function()
                    if remote.Name:find("FishingLuck") or remote.Name:find("Luck") then
                        remote:InvokeServer(Settings.CustomFishingLuck)
                    elseif remote.Name:find("ShinyChance") or remote.Name:find("Shiny") then
                        remote:InvokeServer(Settings.CustomShinyChance)
                    elseif remote.Name:find("MutationChance") or remote.Name:find("Mutation") then
                        remote:InvokeServer(Settings.CustomMutationChance)
                    elseif remote.Name:find("ModifyPlayerStats") then
                        remote:InvokeServer("FishingLuck", Settings.CustomFishingLuck)
                        remote:InvokeServer("ShinyChance", Settings.CustomShinyChance)
                        remote:InvokeServer("MutationChance", Settings.CustomMutationChance)
                    end
                end)
            end
        end
        
        -- Find the player's stats and modify them
        local playerStats = player:FindFirstChild("leaderstats") or player:FindFirstChild("PlayerStats") or player:FindFirstChild("Stats")
        if playerStats then
            -- Modify Fishing Luck
            local fishingLuck = playerStats:FindFirstChild("FishingLuck") or playerStats:FindFirstChild("Luck")
            if fishingLuck and fishingLuck:IsA("NumberValue") then
                fishingLuck.Value = Settings.CustomFishingLuck
            end
            
            -- Modify Shiny Chance
            local shinyChance = playerStats:FindFirstChild("ShinyChance")
            if shinyChance and shinyChance:IsA("NumberValue") then
                shinyChance.Value = Settings.CustomShinyChance
            end
            
            -- Modify Mutation Chance
            local mutationChance = playerStats:FindFirstChild("MutationChance")
            if mutationChance and mutationChance:IsA("NumberValue") then
                mutationChance.Value = Settings.CustomMutationChance
            end
        end
        
        -- Alternative method: Try to find stats in workspace or other locations
        local workspaceStats = workspace:FindFirstChild("PlayerStats") or workspace:FindFirstChild("Stats")
        if workspaceStats then
            local playerFolder = workspaceStats:FindFirstChild(player.Name)
            if playerFolder then
                for _, stat in pairs(playerFolder:GetChildren()) do
                    if stat:IsA("NumberValue") then
                        if stat.Name:lower():find("luck") then
                            stat.Value = Settings.CustomFishingLuck
                        elseif stat.Name:lower():find("shiny") then
                            stat.Value = Settings.CustomShinyChance
                        elseif stat.Name:lower():find("mutation") then
                            stat.Value = Settings.CustomMutationChance
                        end
                    end
                end
            end
        ...
        -- Try modifying GUI display values
        local playerGui = player:FindFirstChild("PlayerGui")
        if playerGui then
            local function findAndModifyStatsDisplay(parent)
                for _, child in pairs(parent:GetChildren()) do
                    if child:IsA("TextLabel") or child:IsA("TextBox") then
                        local text = child.Text:lower()
                        if text:find("fishing luck") and text:find("%+") then
                            child.Text = "Fishing Luck: +" .. Settings.CustomFishingLuck .. "%"
                        elseif text:find("shiny chance") then
                            child.Text = "Shiny Chance: " .. Settings.CustomShinyChance .. "%"
                        elseif text:find("mutation chance") then
                            child.Text = "Mutation Chance: +" .. Settings.CustomMutationChance .. "%"
                        end
                    end
                    if #child:GetChildren() > 0 then
                        findAndModifyStatsDisplay(child)
                    end
                end
            end
            
            -- Search common GUI locations
            local statsGuis = {
                playerGui:FindFirstChild("StatsGui"),
                playerGui:FindFirstChild("Stats"),
                playerGui:FindFirstChild("PlayerStats"),
                playerGui:FindFirstChild("UI"),
                playerGui:FindFirstChild("MainGui")
            }
            
            for _, gui in pairs(statsGuis) do
                if gui then
                    findAndModifyStatsDisplay(gui)
                end
            end
        end
    end)
...
-- Function to continuously apply fishing stats modifications
local function startFishingStatsModifier()
    task.spawn(function()
        while Settings.ModifyFishingStats do
            modifyFishingStats()
...
local function createCompleteGUI()
    ...
    -- Fishing Stats Modifier Frame
    local FishingStatsFrame = Instance.new("Frame")
    FishingStatsFrame.Name = "FishingStatsFrame"
    FishingStatsFrame.Parent = AdvancedListLayoutFrame
    FishingStatsFrame.BackgroundColor3 = Color3.fromRGB(47, 47, 47)
    FishingStatsFrame.BorderSizePixel = 0
    FishingStatsFrame.Size = UDim2.new(0.898, 0, 0.106, 0)
    
    local fishingStatsCorner = Instance.new("UICorner")
    fishingStatsCorner.Parent = FishingStatsFrame

    local FishingStatsText = Instance.new("TextLabel")
    FishingStatsText.Parent = FishingStatsFrame
    FishingStatsText.BackgroundTransparency = 1
    FishingStatsText.Position = UDim2.new(0.030, 0, 0.216, 0)
    FishingStatsText.Size = UDim2.new(0.415, 0, 0.568, 0)
    FishingStatsText.Font = Enum.Font.SourceSansBold
    FishingStatsText.Text = "MODIFY FISHING STATS :"
    FishingStatsText.TextColor3 = Color3.fromRGB(255, 255, 255)
    FishingStatsText.TextScaled = true
    FishingStatsText.TextXAlignment = Enum.TextXAlignment.Left

    local FishingStatsButton = Instance.new("TextButton")
    FishingStatsButton.Name = "FishingStatsButton"
    FishingStatsButton.Parent = FishingStatsFrame
    FishingStatsButton.BackgroundTransparency = 1
    FishingStatsButton.Position = UDim2.new(0.738, 0, 0.108, 0)
    FishingStatsButton.Size = UDim2.new(0.207, 0, 0.784, 0)
    FishingStatsButton.ZIndex = 2
    FishingStatsButton.Font = Enum.Font.SourceSansBold
    FishingStatsButton.Text = "OFF"
    FishingStatsButton.TextColor3 = Color3.fromRGB(255, 255, 255)
    FishingStatsButton.TextScaled = true

    local FishingStatsWarna = Instance.new("Frame")
    FishingStatsWarna.Parent = FishingStatsFrame
    FishingStatsWarna.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    FishingStatsWarna.BorderSizePixel = 0
    FishingStatsWarna.Position = UDim2.new(0.719, 0, 0.135, 0)
    FishingStatsWarna.Size = UDim2.new(0.257, 0, 0.730, 0)
    
    local fishingStatsWarnaCorner = Instance.new("UICorner")
    fishingStatsWarnaCorner.Parent = FishingStatsWarna

    -- Fishing Luck Frame
    local FishingLuckFrame = Instance.new("Frame")
    FishingLuckFrame.Name = "FishingLuckFrame"
    FishingLuckFrame.Parent = AdvancedListLayoutFrame
    FishingLuckFrame.BackgroundColor3 = Color3.fromRGB(47, 47, 47)
    FishingLuckFrame.BorderSizePixel = 0
    FishingLuckFrame.Size = UDim2.new(0.898, 0, 0.106, 0)
    
    local fishingLuckCorner = Instance.new("UICorner")
    fishingLuckCorner.Parent = FishingLuckFrame

    local FishingLuckText = Instance.new("TextLabel")
    FishingLuckText.Parent = FishingLuckFrame
    FishingLuckText.BackgroundTransparency = 1
    FishingLuckText.Position = UDim2.new(0.030, 0, 0.216, 0)
    FishingLuckText.Size = UDim2.new(0.415, 0, 0.568, 0)
    FishingLuckText.Font = Enum.Font.SourceSansBold
    FishingLuckText.Text = "FISHING LUCK (%):"
    FishingLuckText.TextColor3 = Color3.fromRGB(255, 255, 255)
    FishingLuckText.TextScaled = true
    FishingLuckText.TextXAlignment = Enum.TextXAlignment.Left

    local FishingLuckTextBox = Instance.new("TextBox")
    FishingLuckTextBox.Name = "FishingLuckTextBox"
    FishingLuckTextBox.Parent = FishingLuckFrame
    FishingLuckTextBox.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    FishingLuckTextBox.BorderSizePixel = 0
    FishingLuckTextBox.Position = UDim2.new(0.719, 0, 0.135, 0)
    FishingLuckTextBox.Size = UDim2.new(0.257, 0, 0.730, 0)
    FishingLuckTextBox.ZIndex = 3
    FishingLuckTextBox.Font = Enum.Font.SourceSansBold
    FishingLuckTextBox.PlaceholderText = "175"
    FishingLuckTextBox.Text = ""
    FishingLuckTextBox.TextColor3 = Color3.fromRGB(255, 255, 255)
    FishingLuckTextBox.TextScaled = true
    
    local fishingLuckTextCorner = Instance.new("UICorner")
    fishingLuckTextCorner.Parent = FishingLuckTextBox

    -- Shiny Chance Frame
    local ShinyChanceFrame = Instance.new("Frame")
    ShinyChanceFrame.Name = "ShinyChanceFrame"
    ShinyChanceFrame.Parent = AdvancedListLayoutFrame
    ShinyChanceFrame.BackgroundColor3 = Color3.fromRGB(47, 47, 47)
    ShinyChanceFrame.BorderSizePixel = 0
    ShinyChanceFrame.Size = UDim2.new(0.898, 0, 0.106, 0)
    
    local shinyChanceCorner = Instance.new("UICorner")
    shinyChanceCorner.Parent = ShinyChanceFrame

    local ShinyChanceText = Instance.new("TextLabel")
    ShinyChanceText.Parent = ShinyChanceFrame
    ShinyChanceText.BackgroundTransparency = 1
    ShinyChanceText.Position = UDim2.new(0.030, 0, 0.216, 0)
    ShinyChanceText.Size = UDim2.new(0.415, 0, 0.568, 0)
    ShinyChanceText.Font = Enum.Font.SourceSansBold
    ShinyChanceText.Text = "SHINY CHANCE (%):"
    ShinyChanceText.TextColor3 = Color3.fromRGB(255, 255, 255)
    ShinyChanceText.TextScaled = true
    ShinyChanceText.TextXAlignment = Enum.TextXAlignment.Left

    local ShinyChanceTextBox = Instance.new("TextBox")
    ShinyChanceTextBox.Name = "ShinyChanceTextBox"
    ShinyChanceTextBox.Parent = ShinyChanceFrame
    ShinyChanceTextBox.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    ShinyChanceTextBox.BorderSizePixel = 0
    ShinyChanceTextBox.Position = UDim2.new(0.719, 0, 0.135, 0)
    ShinyChanceTextBox.Size = UDim2.new(0.257, 0, 0.730, 0)
    ShinyChanceTextBox.ZIndex = 3
    ShinyChanceTextBox.Font = Enum.Font.SourceSansBold
    ShinyChanceTextBox.PlaceholderText = "4"
    ShinyChanceTextBox.Text = ""
    ShinyChanceTextBox.TextColor3 = Color3.fromRGB(255, 255, 255)
    ShinyChanceTextBox.TextScaled = true
    
    local shinyChanceTextCorner = Instance.new("UICorner")
    shinyChanceTextCorner.Parent = ShinyChanceTextBox

    -- Mutation Chance Frame
    local MutationChanceFrame = Instance.new("Frame")
    MutationChanceFrame.Name = "MutationChanceFrame"
    MutationChanceFrame.Parent = AdvancedListLayoutFrame
    MutationChanceFrame.BackgroundColor3 = Color3.fromRGB(47, 47, 47)
    MutationChanceFrame.BorderSizePixel = 0
    MutationChanceFrame.Size = UDim2.new(0.898, 0, 0.106, 0)
    
    local mutationChanceCorner = Instance.new("UICorner")
    mutationChanceCorner.Parent = MutationChanceFrame

    local MutationChanceText = Instance.new("TextLabel")
    MutationChanceText.Parent = MutationChanceFrame
    MutationChanceText.BackgroundTransparency = 1
    MutationChanceText.Position = UDim2.new(0.030, 0, 0.216, 0)
    MutationChanceText.Size = UDim2.new(0.415, 0, 0.568, 0)
    MutationChanceText.Font = Enum.Font.SourceSansBold
    MutationChanceText.Text = "MUTATION CHANCE (%):"
    MutationChanceText.TextColor3 = Color3.fromRGB(255, 255, 255)
    MutationChanceText.TextScaled = true
    MutationChanceText.TextXAlignment = Enum.TextXAlignment.Left

    local MutationChanceTextBox = Instance.new("TextBox")
    MutationChanceTextBox.Name = "MutationChanceTextBox"
    MutationChanceTextBox.Parent = MutationChanceFrame
    MutationChanceTextBox.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    MutationChanceTextBox.BorderSizePixel = 0
    MutationChanceTextBox.Position = UDim2.new(0.719, 0, 0.135, 0)
    MutationChanceTextBox.Size = UDim2.new(0.257, 0, 0.730, 0)
    MutationChanceTextBox.ZIndex = 3
    MutationChanceTextBox.Font = Enum.Font.SourceSansBold
    MutationChanceTextBox.PlaceholderText = "0"
    MutationChanceTextBox.Text = ""
    MutationChanceTextBox.TextColor3 = Color3.fromRGB(255, 255, 255)
    MutationChanceTextBox.TextScaled = true
    
    local mutationChanceTextCorner = Instance.new("UICorner")
    mutationChanceTextCorner.Parent = MutationChanceTextBox
    ...
    connections[#connections + 1] = MinimizeBtn.MouseButton1Click:Connect(function()
        ...
        if isMinimized then
            -- Minimize GUI - hide all frames except title bar
            ...
            -- Resize main frame to just show title bar
            FrameUtama:TweenSize(
                UDim2.new(0.542, 0, 0.12, 0),
                "Out", "Quad", 0.3, true
            )
            MinimizeBtn.Text = "+"
            createNotification("📦 GUI Minimized", Color3.fromRGB(255, 165, 0))
        else
            -- Restore GUI
            ...
            FrameUtama:TweenSize(
                UDim2.new(0.542, 0, 0.650, 0),
                "Out", "Quad", 0.3, true,
                function()
                    -- Show current panel after animation
                    if Tittle.Text == "MAIN" then
                        MainFrame.Visible = true
                    elseif Tittle.Text == "PLAYER" then
                        PlayerFrame.Visible = true
                    elseif Tittle.Text == "TELEPORT" then
                        Teleport.Visible = true
                    elseif Tittle.Text == "SPAWN BOAT" then
                        SpawnBoatFrame.Visible = true
                    elseif Tittle.Text == "SECURITY" then
                        SecurityFrame.Visible = true
                    elseif Tittle.Text == "ADVANCED" then
    ...
    -- Fishing Stats Modifier button
    connections[#connections + 1] = FishingStatsButton.MouseButton1Click:Connect(function()
        Settings.ModifyFishingStats = not Settings.ModifyFishingStats
        FishingStatsButton.Text = Settings.ModifyFishingStats and "ON" or "OFF"
        FishingStatsWarna.BackgroundColor3 = Settings.ModifyFishingStats and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(0, 0, 0)
        if Settings.ModifyFishingStats then
            startFishingStatsModifier()
            createNotification("🎣 Fishing Stats Modifier enabled!", Color3.fromRGB(0, 255, 0))
        else
            createNotification("🎣 Fishing Stats Modifier disabled!", Color3.fromRGB(200, 0, 0))
        end
    end)

    -- Fishing Luck TextBox
    connections[#connections + 1] = FishingLuckTextBox.FocusLost:Connect(function(enterPressed)
        if enterPressed then
            local value = tonumber(FishingLuckTextBox.Text)
            if value and value >= 0 and value <= 1000 then
                Settings.CustomFishingLuck = value
                createNotification("🍀 Fishing Luck set to " .. value .. "%", Color3.fromRGB(255, 215, 0))
                FishingLuckTextBox.Text = ""
            else
                createNotification("❌ Invalid luck! Use 0-1000", Color3.fromRGB(255, 0, 0))
                FishingLuckTextBox.Text = ""
            end
        end
    end)

    -- Shiny Chance TextBox
    connections[#connections + 1] = ShinyChanceTextBox.FocusLost:Connect(function(enterPressed)
        if enterPressed then
            local value = tonumber(ShinyChanceTextBox.Text)
            if value and value >= 0 and value <= 100 then
                Settings.CustomShinyChance = value
                createNotification("✨ Shiny Chance set to " .. value .. "%", Color3.fromRGB(255, 215, 0))
                ShinyChanceTextBox.Text = ""
            else
                createNotification("❌ Invalid chance! Use 0-100", Color3.fromRGB(255, 0, 0))
                ShinyChanceTextBox.Text = ""
            end
        end
    end)

    -- Mutation Chance TextBox
    connections[#connections + 1] = MutationChanceTextBox.FocusLost:Connect(function(enterPressed)
        if enterPressed then
            local value = tonumber(MutationChanceTextBox.Text)
            if value and value >= 0 and value <= 100 then
                Settings.CustomMutationChance = value
                createNotification("🧬 Mutation Chance set to " .. value .. "%", Color3.fromRGB(128, 0, 255))
                MutationChanceTextBox.Text = ""
            else
                createNotification("❌ Invalid chance! Use 0-100", Color3.fromRGB(255, 0, 0))
                MutationChanceTextBox.Text = ""
    ...
    connections[#connections + 1] = UserInputService.InputBegan:Connect(function(input, gameProcessed)
        ...
        if input.KeyCode == CONFIG.HOTKEY then
            ...
            ZayrosFISHIT.Enabled = not isHidden
        end
    end)
    ...
    print("🎣 NEW: Fishing Stats Modifier - Control Luck, Shiny & Mutation Chance!")
    print("💎 Configure custom fishing stats in Advanced tab")
